...
public struct ReplicatingDictionary<Key, Value> where Key: Hashable {
    ...
    public var values: [Value] {
        existingKeyValuePairs.map { $0.value.value }
    }
    
    public var keys: [Key] {
        existingKeyValuePairs.map { $0.key }
    }
    ...
    public subscript(key: Key) -> Value? {
        get {
            guard let container = valueContainersByKey[key], !container.isDeleted else {
                return nil
            }
...
extension ReplicatingDictionary where Value: Replicable {
    ...
    /// Instead of just choosing one value or the other, we can merge the values themselves.
    /// This merge method does exactly that.
    public func merged(with other: ReplicatingDictionary) -> ReplicatingDictionary {
